...
\begin{align}
 SSA &= \sum_{i=1}^{n_f} n_{o}(\bar{y}_{i.}-\bar{y}_{..})^2
\end{align}
...
\begin{align}
 SSE &= \sum_{i=1}^{n_f}\sum_{j=1}^{n_o} (y_{ij} - y_{i.})^2
\end{align}
...
\begin{align}
 E(MSE) &=  \sigma^2_{e} \\ \notag
 E(MSA) &=  \sigma^2_{e} + n_o\sigma^2_{f}  \notag
\end{align}
...
possible values. For example, if the observation is the distance a rock can be thrown, then a continuous factor would be the weight of the rock. If the observation is the rate of growth, then a continuous factor would be the amount of feed eaten.
...
If the number of levels of a categorical variables is small or limited to a fixed number, and inferences about a factor are going to be limited to that set of levels, and to no others, then that factor is usually fixed. In other words, if a new sample of observations is made (from a new experiment), and the same levels of that factor are in both samples, then the factor is usually fixed. Continuous variables are usually fixed too (but not always).

If the number of levels of a categorical variable is large, then that factor may be random. If the inferences about a factor are going to be made to an entire population of conceptual levels, and if the levels of a factor are a sample from an infinitely large population, then that factor is usually random. In other words, if a new sample of observations were made (from a new experiment), and the levels are completely different between the two samples, then the factor is usually random.
